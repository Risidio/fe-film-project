...
export async function POST(req: NextRequest) {
    try {
        ...
        const { assetURI } = body;
        if (!assetURI) {
            return NextResponse.json({ error: "assetURI is required" }, { status: 400 });
        }
        const result = await filmRightsService.getAssetCreator(assetURI);
        return NextResponse.json(result, { status: 200 });
    } catch (error: any) {
        console.error("Error in getAssetCreator:", error);
